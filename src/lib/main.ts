...
export async function askQuestion(question: string, projectId: string) {
  ...
  // 2. Get relevant files from DB based on vector similarity
...
`,
 ...
 // 5. Collect the final output from stream
  let finalAnswer = ''
...
;(async () => {
  const { output } = await askQuestion(
    'what model are there in the schema?',
    'cmef33mvp0000uy7fnw0o2wzh'
  )
